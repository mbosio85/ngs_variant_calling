...
def create_workflow_summary(summary) {
    ...
    yaml_file.text  = """
    id: 'nf-core-nftest-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/nftest Workflow Summary'
...
workflow.onComplete {
    ...
    // Set up the e-mail variables
    def subject = "[nf-core/nftest] Successful: $workflow.runName"
    if (!workflow.success) {
      subject = "[nf-core/nftest] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    email_fields['version'] = workflow.manifest.version
    email_fields['runName'] = custom_runName ?: workflow.runName
    ...
    try {
        if (workflow.success) {
            mqc_report = multiqc_report.getVal()
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/nftest] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/nftest] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
          ...
          // Try to send HTML e-mail using sendmail
          [ 'sendmail', '-t' ].execute() << sendmail_html
          log.info "[nf-core/nftest] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
          // Catch failures and try with plaintext
          [ 'mail', '-s', subject, email_address ].execute() << email_txt
          log.info "[nf-core/nftest] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "${c_purple}[nf-core/nftest]${c_green} Pipeline completed successfully${c_reset}"
    } else {
        checkHostname()
        log.info "${c_purple}[nf-core/nftest]${c_red} Pipeline completed with errors${c_reset}"
    }
